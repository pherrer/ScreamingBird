...
During this project, Paula’s role focused on completing the core functionality that made the game interactive and responsive. She implemented the noise signal processing logic, ensuring that microphone input was correctly detected and translated into a usable control signal for gameplay. This involved tuning thresholds, timing, and sampling behavior so that background noise was filtered out and intentional sound (like claps or voice input) could reliably trigger the bird’s movement. In addition to the noise control, Paula handled the physics-based movement of the bird, including gravity-like falling behavior when no input is detected and upward motion when a noise event occurs. She also performed extensive parameter testing across the programs, adjusting constants such as timing intervals, sensitivity levels, and movement increments to achieve smooth and playable behavior. This testing was critical to balancing responsiveness and stability, making the final game feel natural rather than erratic when running on the Nexys board. Finally, she helped verify that the monitor correctly displayed the game visuals through the VGA output, confirming that the bird movement, obstacles, and overall gameplay updated smoothly and consistently on the screen. On GitHub, Paula focused on writing the project introduction, functionality, and how the system operates at a high level. This helped provide context for the code and made the repository more understandable to someone viewing it for the first time. In addition, Paula documented the code modifications and clearly referenced the original projects used as a foundation, including the noise detection and Flappy Bird VGA designs. She made sure to explain what was reused, what was changed, and why those changes were necessary for this project. Paula began the project by combining code from the previous projects into a single working framework. After establishing this base, she implemented a new noise detection file designed to the needs of the game and then focused on adding the physics behavior, which is also the most difficult part of this project. It was a completely new feature with no prior reference code to rely on, and the logic required careful timing and state control. This made the coding process more complex and time-consuming, but it was essential for creating smooth and realistic gameplay.

Yuning's responsibility include designing all the diagrams and FSM according to the programs, modifying the codes from the flappy bird project, and Nexys board analysis. The system diagrams and finite state machine (FSM) based directly on how the provided VHDL modules operate, ensuring that the game flow, noise detection, and display logic were clearly defined before full integration. Yuning suggested, modified, and combined code from previous projects, specifically building on the Flappy Bird VGA game design. This involved adapting the frame-synchronized flap signal so the bird movement aligned correctly with the VGA refresh timing and responded properly to noise input. Yuning also handled the Nexys board analysis, mapping all inputs and outputs to the correct hardware pins using the XDC file and validating the design on real hardware. By testing the microphone input, VGA display, and seven-segment score output directly on the board, Yuning made sure the system worked as expected not just in simulation, but in actual deployment. On GitHub, Yuning focused on completing the expected system behavior documentation, including the FSM, diagrams, overall system summary (besides the modifications), and the detailed input/output descriptions. Yuning started by analyzing the programs originally created by Paula and used them as the foundation for designing the FSM and system diagrams, while also identifying the correct Nexys pins needed for accurate execution. The most challenging part of the project was working with the noise input logic, since the microphone data transfer and signal processing are more complex and time-consuming than the rest of the system, requiring multiple iterations to get stable and reliable behavior.
